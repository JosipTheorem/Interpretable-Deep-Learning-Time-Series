...
def train_and_evaluate(
    ...
    return test_loss, test_mae, train_losses, val_losses, f_test, c_test, debug_info, test_inputs, test_targets, model
...
def _sample_std(values, axis=None):
...
def collect_multiple_runs(
    ...
    for run in range(n_runs):
        ...
        # Run the model
        test_loss, test_mae, train_losses, val_losses, f_test, c_test, debug_info, test_inputs, test_targets, model = train_and_evaluate(
            time_series,
            window_size=window_size,
            temperature=temperature,
            order=order,
        ...
        for i in f_test.keys():
            alpha_values = f_test[i] * c_test[i]
            alpha_seq[i] = alpha_values
            if i == 0:
                alpha_bias = alpha_values.mean(0)  # Mean of bias alpha coefficients
                alpha_bias_std = alpha_values.std(0)  # Std of bias alpha coefficients
            else:
                alpha[i] = alpha_values.mean(0)  # Mean of regular alpha coefficients
                alpha_std[i] = alpha_values.std(0)  # Std of regular alpha coefficients
        ...
        del model, test_inputs, test_targets, f_test, c_test, debug_info
    ...
    if verbose:
        ...
        print(f"Time taken for {n_runs} run(s): {elapsed_time:.2f} seconds")
    
    # Add summary statistics across runs
    test_losses = [results[f'run_{i+1}']['test_loss'] for i in range(n_runs)]
    test_maes = [results[f'run_{i+1}']['test_mae'] for i in range(n_runs)]
    results['summary'] = {
        'mean_test_loss': np.mean(test_losses),
        'std_test_loss': _sample_std(test_losses),
        'min_test_loss': np.min(test_losses),
        'max_test_loss': np.max(test_losses),
        'mean_test_mae': np.mean(test_maes),
        'std_test_mae': _sample_std(test_maes),
        'min_test_mae': np.min(test_maes),
        'max_test_mae': np.max(test_maes),
...
def evaluate_window_sizes(time_series, window_sizes, order, config, temperature=1., verbose=True):
    ...
    for window_size in window_sizes:
        ...
        test_loss, _, train_losses, val_losses, _, _, _, _, _, _ = train_and_evaluate(time_series, 
                                                                          window_size=window_size,
                                                                          temperature=temperature,
                                                                          order=order,
                                                                          config=config)
...
def calculate_multiple_run_statistics(results: Dict) -> Dict[str, Dict[str, np.ndarray]]:
    ...
    for key in alpha_keys:
        ...
        stats['alpha'][key] = {
            ...
            'std': _sample_std(alphas, axis=0)
        }
        stats['beta'][key] = {
            'mean': np.mean(betas, axis=0),
            'std': _sample_std(betas, axis=0)
        }
    ...
    stats['alpha_bias']['std'] = _sample_std(alpha_biases, axis=0)
    ...
    for key in alpha_keys:  # Same keys as alpha
        ...
        stats['f'][key] = {
            ...
            'std': _sample_std(f_means, axis=0)
        }
        stats['c'][key] = {
            'mean': np.mean(c_means, axis=0),
            'std': _sample_std(c_means, axis=0)
        }
