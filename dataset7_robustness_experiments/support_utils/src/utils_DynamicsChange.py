...
def dataset(len_of_timeseries, ground_truth_alpha, bias, noise_frequency=0, mu=0, sigma=1, burn_in=1000, seed=None, nonlinear_function=None,init_state=None):
    """
    ...
    # Initialize all values with Gaussian variates N(0,1) - sets initial values, and array shape (kreiram random vrijednost za svaki broj u 21000 koraka)
    ...
    if init_state is not None:
        ...
        assert init_state.shape == (no_of_timeseries, window_length), \
            f"init_state must have shape ({no_of_timeseries}, {window_length}), got {init_state.shape}"
        time_series[:, :window_length] = init_state
        
    
    # Generate noise mask only for values after initial window
    noise_mask = (np.random.rand(no_of_timeseries, total_length - window_length) < noise_frequency) #(random vrj od 0-1 za svaki broj i samo oni manji od f postanju True)
    # Add noise as Gaussian variates N(mu,sigma) with mask 
    noise_after_window = np.random.normal(mu, sigma, (no_of_timeseries, total_length - window_length)) * noise_mask #random vrj šuma gdje samo oni koji su true prežive (njih 30%)
    
    # Only apply masked noise after the initial window
    time_series[:, window_length:] = torch.tensor(noise_after_window, dtype=torch.float32)  ### stavi vrijednost šuma na svaku vrijednost za svaki time step osim prvih L gjde ostaju oni random izmedu (0,1)
...
def plot_ts(dataset_sample, dataset_name="Dataset", alpha=0.7, save=False):
...
def train_and_evaluate(
    ...
    test_windowed_dataset = create_windowed_dataset(different_time_series, window_size)
...
def collect_multiple_runs(
    n_runs: int,
    time_series: torch.Tensor,
    different_time_series: torch.Tensor,
    window_size: int,
    temperature: float,
    order: List[int] = None,
    config=None,
    ...
    for run in range(n_runs):
        ...
        test_loss, train_losses, val_losses, f_test, c_test, debug_info, _, _, _ = train_and_evaluate(
            ...
            different_time_series=different_time_series,
            window_size=window_size,
            temperature=temperature,
            order=order,
            config=train_config
        ...
        for i in f_test.keys():
            if i == 0:
                ...
                alpha_bias_full = (f_test[0] * c_test[0])
            else:
                ...
                alpha_full[i] = (f_test[i] * c_test[i])  
                f_full[i] = f_test[i]  
                c_full[i] = c_test[i] 
                
                alpha_first[i] = (f_test[i][:half] * c_test[i][:half]).mean(0)  
                alpha_first_std[i] = (f_test[i][:half] * c_test[i][:half]).std(0)  
                alpha_second[i] = (f_test[i][half:] * c_test[i][half:]).mean(0)  
                alpha_second_std[i] = (f_test[i][half:] * c_test[i][half:]).std(0)
        ...
        results[f'run_{run + 1}'] = {
            ...
            'f_means': f_means,
            'c_means': c_means,
            'alpha_bias_full': alpha_bias_full,
            'alpha_full': alpha_full,
            'f_full': f_full,
            'c_full': c_full,
            'alpha_first': alpha_first,
            'alpha_first_std': alpha_first_std,
            'alpha_second': alpha_second,
            'alpha_second_std': alpha_second_std,          
        }
...
def plot_alphas(alpha, ground_truth_alpha, cmap='seismic', figsize=(6, 5), font_size=14, title=r'\alpha',space=0.1,cbar_font=11, force_12=False, save=False):
    """
...
def calculate_multiple_run_statistics(results: Dict) -> Dict[str, Dict[str, np.ndarray]]:
    """
    Calculate mean and std of alpha, alpha_first, alpha_second, alpha_bias,
    f_means, c_means, beta across runs.
    """
    ...
    # get alpha keys (usually [1] for linear)
    ...
    for key in alpha_keys:
        ...
        stats['alpha'][key] = {
            ...
            'std':  alphas.std(axis=0)
        }

        # α_first
        alpha_first = np.stack([results[f'run_{i+1}']['alpha_first'][key] for i in range(n_runs)], axis=0)
        stats['alpha_first'][key] = {
            'mean': alpha_first.mean(axis=0),
            'std':  alpha_first.std(axis=0)
        }

        # α_second
        alpha_second = np.stack([results[f'run_{i+1}']['alpha_second'][key] for i in range(n_runs)], axis=0)
        stats['alpha_second'][key] = {
            'mean': alpha_second.mean(axis=0),
            'std':  alpha_second.std(axis=0)
        }

        # compute β for each run
        betas = np.empty((n_runs,) + alphas.shape[1:-1])
        for i in range(n_runs):
            beta_tilde = np.abs(alphas[i]).sum(axis=-1)
            betas[i] = beta_tilde / beta_tilde.sum(axis=1, keepdims=True)
        ...
        stats['f'][key] = {
            'mean': f_means.mean(axis=0),
            'std':  f_means.std(axis=0)
        }

        # c means
        c_means = np.stack([results[f'run_{i+1}']['c_means'][key] for i in range(n_runs)], axis=0)
        stats['c'][key] = {
            'mean': c_means.mean(axis=0),
            'std':  c_means.std(axis=0)
        }

    # ---------- α_bias ----------
    alpha_biases = np.stack([results[f'run_{i+1}']['alpha_bias'] for i in range(n_runs)], axis=0)
    stats['alpha_bias']['mean'] = alpha_biases.mean(axis=0)
    stats['alpha_bias']['std']  = alpha_biases.std(axis=0)
