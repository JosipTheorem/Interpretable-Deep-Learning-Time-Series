...
def introduce_missing_data(time_series, missing_ratio, seed=1000):
    """
    Randomly set a fraction of entries to NaN to simulate missing data.
    Args:
        time_series: torch.Tensor of shape (N, T)
        missing_ratio: float in [0, 1], fraction of entries to drop
    Returns:
        masked_ts: numpy array with NaNs
        mask: numpy array of booleans (True = observed)
    """
    np.random.seed(seed)
    ts_np = time_series.numpy().copy()
    mask = np.random.rand(*ts_np.shape) > missing_ratio
    ts_np[~mask] = np.nan
    return ts_np, mask


def impute_missing(ts_np, method="linear"):
    """
    Imputes NaN values using different methods.
    """
    ts_filled = np.zeros_like(ts_np)
    for i in range(ts_np.shape[0]):  # per time series
        s = pd.Series(ts_np[i])
        if method == "linear":
            ...
            s = s.fillna(s.mean())
        ts_filled[i] = s.values
    return torch.tensor(ts_filled, dtype=torch.float32)
...
def split_time_series(time_series, train_ratio=0.6, val_ratio=0.2, window_size=5, train_impute=False, test_impute=False, missing_ratio=0, impute_method="linear"):
    """
    ...
    if train_impute:
        ts_np, _ = introduce_missing_data(train_series, missing_ratio)
        train_series = impute_missing(ts_np, method=impute_method)
        
        ts_np, _ = introduce_missing_data(val_series, missing_ratio)
        val_series = impute_missing(ts_np, method=impute_method)
        
    if test_impute:
        ts_np, _ = introduce_missing_data(test_series, missing_ratio)
        test_series = impute_missing(ts_np, method=impute_method)
...
def collect_multiple_runs(
    ...
    if config is None:
        train_config = {
        'verbose': True,
        'device' : device, ### tu bi inace trebao failat, ali se nije vidjelo jer config nije bio None do sad
        'learning_rate': 1e-3,
        'scheduler_patience': 5,
        'early_stopping_modifier': 2,
...
def evaluate_window_sizes(
    ...
    for window_size in window_sizes:
        ...
        test_loss, train_losses, val_losses, _, _, _, _, _, _ = train_and_evaluate(
                                                                                    time_series,
                                                                                    window_size=window_size,
                                                                                    temperature=temperature,
                                                                                    order=order,
